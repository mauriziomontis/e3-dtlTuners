...
#-d   \param HW_DESC Hardware descriptor, i.e. EL2819
...
#-d   \note Example call:
#-d   \code
#-d    ${SCRIPTEXEC} "${dtlTuner_DIR}aliasMotorSwitchOutput.cmd", "SLAVE_ID=1, HW_DESC=EL2819, BIT_IN=0, BIT_OUT=1, SECTION_NAME=DTL-010, DEVICE_POS=001"
#-d   \endcode
#-d */

dbLoadRecords(ecmcGenericAlias.db,"NAME=${SM_PREFIX}ec${MASTER_ID}-s${SLAVE_ID}-${HW_DESC}-BO${BIT_IN}, ALIAS=${SECTION_NAME}:EMR-GS-00${DEVICE_POS}:24VPS_OnCmd") 
dbLoadRecords(ecmcGenericAlias.db,"NAME=${SM_PREFIX}ec${MASTER_ID}-s${SLAVE_ID}-${HW_DESC}-BO${BIT_IN}-Stat, ALIAS=${SECTION_NAME}:EMR-GS-00${DEVICE_POS}:24VPS_OnStat") 
dbLoadRecords(ecmcGenericAlias.db,"NAME=${SM_PREFIX}ec${MASTER_ID}-s${SLAVE_ID}-${HW_DESC}-BO${BIT_IN}-RB, ALIAS=${SECTION_NAME}:EMR-GS-00${DEVICE_POS}:24VPS_OnCmd-RB") 
...
dbLoadRecords(ecmcGenericAlias.db,"NAME=${SM_PREFIX}ec${MASTER_ID}-s${SLAVE_ID}-${HW_DESC}-BO${BIT_OUT}, ALIAS=${SECTION_NAME}:EMR-GS-01${DEVICE_POS}:24VPS_OnCmd") 
dbLoadRecords(ecmcGenericAlias.db,"NAME=${SM_PREFIX}ec${MASTER_ID}-s${SLAVE_ID}-${HW_DESC}-BO${BIT_OUT}-Stat, ALIAS=${SECTION_NAME}:EMR-GS-01${DEVICE_POS}:24VPS_OnStat") 
dbLoadRecords(ecmcGenericAlias.db,"NAME=${SM_PREFIX}ec${MASTER_ID}-s${SLAVE_ID}-${HW_DESC}-BO${BIT_OUT}-RB, ALIAS=${SECTION_NAME}:EMR-GS-01${DEVICE_POS}:24VPS_OnCmd-RB") 
